...
import { rumors as initialRumors } from '../../data/mockData'

export default function RumorsAdmin() {
  const { t } = useTranslation()
  const [rows, setRows]         = useState(initialRumors)
    ...
    <div>
      ...
      <AdminModal isOpen={modalOpen} title={editing ? t('admin.editItem') : t('admin.addNew')} onClose={() => setModal(false)}>
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-bold text-primary mb-1">{t('admin.col.status')}</label>
            <select name="status" defaultValue={editing?.status ?? 'unverified'}
              className="w-full border border-[#00334a]/20 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-primary">
              <option value="confirmed">{t('status.confirmed')}</option>
              <option value="false">{t('status.false')}</option>
